"""
simulation/tests/smoke_pf_lifecycle.py
=======================================
pf_models.py lifecycle smoke test:
  - 각 pf forecaster 를 instantiate
  - BaseForecaster 인터페이스 fit(X,y) -> predict(X_test) 호출
  - 출력 shape/range 가 WF-CV 파이프라인과 호환되는지 확인

현실 shape (n_train=222, n_test=12, p=30) 로 테스트.
EPOCHS=5 로 축소 (smoke 목적 -- 품질 아닌 interface 확인).

직접 실행:
  .venv\\Scripts\\python.exe simulation/tests/smoke_pf_lifecycle.py
"""
...
def main():
    from simulation.models.modern_ts import (
        PfTFTForecaster, PfNBeatsForecaster, PfNHiTSForecaster,
        PfTiDEForecaster, PfRNNForecaster, PfDeepARForecaster,
    )

    # 모든 pf 모델에 smoke EPOCHS override
    classes = [
        PfTFTForecaster, PfNBeatsForecaster, PfNHiTSForecaster,
        PfTiDEForecaster, PfRNNForecaster, PfDeepARForecaster,
    ]
    ...
    for cls in classes:
        ...
        try:
            ...
            print(f"[{status:>3}] {name:<12} time={total:5.1f}s "
                  f"shape={yhat.shape} nonneg={ok_nonneg} finite={ok_finite} "
                  f"range=[{yhat.min():.2f},{yhat.max():.2f}]")
            results.append({"model": name, "status": status, "time": total,
                            "shape": yhat.shape,
                            "range": (float(yhat.min()), float(yhat.max()))})
        except Exception as e:
            total = time.time() - t0
            print(f"[FAIL] {name:<12} time={total:5.1f}s  "
